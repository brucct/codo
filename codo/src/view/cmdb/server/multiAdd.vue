...
<Modal width="600px;" v-model="dialog.show"  :title="dialog.title" :loading=true :footer-hide=true @on-cancel="handleCancel">
    <Form ref="formData3" :model="formData" :label-width="80">
        <Alert type="success">格式1: [hostname] [ip] [port] [管理用户]</Alert>
        <Alert type="success">格式2: [hostname] [ip] [port] [username] [password]</Alert>
        <!-- <FormItem label="批量添加"> -->
            <Input v-model="formData.data" type="textarea" :autosize="{minRows: 5,maxRows: 10}" placeholder="test01 10.0.0.1 22 ops"></Input>
        <!-- </FormItem> -->
        <!-- <FormItem> -->
            <br><br>
...
<script>
    import { multiAdd } from '@/api/cmdb/server'
    export default {
        name: 'multiadd',
        data () {
            return {
