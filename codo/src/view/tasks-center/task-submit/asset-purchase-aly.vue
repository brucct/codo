      ...
      <Col span="15">
       <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
        ...
        <Row>
          ...
          <Col span="2" style="text-align: center">_</Col>
          <Col span="7">
            <FormItem label="OSS对象存储" prop="OSS">
              <RadioGroup v-model="formValidate.OSS">
                <Radio label="true">是</Radio>
                <Radio label="false">否</Radio>
              </RadioGroup>
           </FormItem>
        ...
        <Alert>
          <div style="color: #ed4014">
            ...
            <h3>4. 主机名称必须符合规范，例如：FT-HK_ALY-01-Web01 第一位代表项目简称，第二位代表区域是阿里云的香港，第三位代表这是第一组，最后一位代表运行的服务。（暂时不开通强规范）</h3>
            <h3>5. 购买的类型要参考阿里云的文档。<br>
...
<script>
import { getTaglist, operationAssetPurchaseALY} from '@/api/task-other'
  export default {
    data () {
      return {
        allTagList: [],
  ...
  methods: {
    handleSubmit (name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          setTimeout(() => {
            operationAssetPurchaseALY(this.formValidate, "post").then(res => {
              if (res.data.code === 0) {
