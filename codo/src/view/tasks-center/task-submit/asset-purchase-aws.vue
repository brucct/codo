       ...
       <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
        ...
        <FormItem label="环境-工具" prop="host_env">
          <CheckboxGroup v-model="formValidate.host_env">
            ...
            <Checkbox label="kinesis"></Checkbox>
        ...
        <Row>
          ...
          <Col span="2" style="text-align: center">_</Col>
          <Col span="7">
            <FormItem label="S3对象存储" prop="S3">
              <RadioGroup v-model="formValidate.S3">
                <Radio label="true">是</Radio>
                <Radio label="false">否</Radio>
              </RadioGroup>
           </FormItem>
        ...
        <Alert>
          <div style="color: #ed4014">
            ...
            <h3>4. 主机名称必须符合规范，例如：OPS-VA-test01-ELK01 第一位代表项目属于运维，第二位代表区域是亚马逊的弗吉尼亚，第三位代表这是第一组测试，最后一位代表运行的服务。（暂时不开通强规范）</h3>
            <h3>5. 购买的类型要参考AWS的文档。<br>
            ...
            <h3>6. AWS必须使用弹性IP，正式环境必须使用负载均衡地址，域名直接关联负载均衡地址。 </h3>
...
<script>
import { getTaglist, operationAssetPurchaseAWS} from '@/api/task-other'
  export default {
    data () {
      return {
        allTagList: [],
  ...
  methods: {
    handleSubmit (name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          setTimeout(() => {
            operationAssetPurchaseAWS(this.formValidate, "post").then(res => {
              if (res.data.code === 0) {
