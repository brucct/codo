...
class ServerHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        with DBContext('r') as session:
            ...
            exist_ip = session.query(Server.id).filter(Server.public_ip == public_ip).first()
        if exist_id or exist_ip:
            return self.write(dict(code=-2, msg='不要重复记录'))

        with DBContext('w', None, True) as session:
            new_server = Server(hostname=hostname, public_ip=public_ip, private_ip=private_ip, port=int(port), idc=idc,
    ...
    def put(self, *args, **kwargs):
        ...
        with DBContext('w', None, True) as session:
            ...
            session.query(Server).filter(Server.id == server_id).update({Server.hostname: hostname,
    ...
    def delete(self, *args, **kwargs):
        ...
        with DBContext('w', None, True) as session:
            if server_id:
                ...
                session.query(ServerDetail).filter(ServerDetail.public_ip == server_info.public_ip).delete(synchronize_session=False)
                ...
                for i in id_list:
                    ...
                    session.query(ServerDetail).filter(ServerDetail.public_ip == server_info.public_ip).delete(
