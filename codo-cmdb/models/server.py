...
class Server(Base):
    ...
    public_ip = Column('public_ip', String(32), nullable=False)  # 公网IP
    private_ip = Column('private_ip', String(32))  # 私网IP
...
class ServerDetail(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)  # ID自增长
    public_ip = Column('public_ip', String(32), nullable=False)  # 公网IP,根据上面的IP进行关联
    private_ip = Column('private_ip',String(32)) #私网IP
...
class AssetErrorLog(Base):
    ...
    ### 自动推送资产。错误日志信息
    id = Column('id', Integer, primary_key=True, autoincrement=True)  # ID自增长
    public_ip = Column('public_ip', String(32), nullable=False)  # IP
    private_ip = Column('private_ip', String(32), nullable=False) # private_ip
