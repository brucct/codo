    ...
    def sync_cmdb(self):
        ...
        with DBContext('w') as session:
            for server in server_list:
                ...
                exist_ip = session.query(Server).filter(Server.public_ip == public_ip).first()

                if not exist_hostname and not exist_ip:
                    new_serve = Server(public_ip=public_ip, private_ip=private_ip, hostname=hostname, port=22, idc=self.account, region=region,
                                       state=self.state,
