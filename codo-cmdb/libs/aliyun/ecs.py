            ...
            for server in server_list:
                ...
                if exist_hostname:
                    session.query(Server).filter(Server.hostname == hostname).update(
                        {Server.public_ip: public_ip, Server.private_ip: private_ip, Server.idc: self.account, Server.region: region,
                         Server.admin_user: self.default_admin_user})

                else:
                    if os_type == 'windows':
                        # windows机器不绑定管理用户，资产信息只是平台拿到的一些基础信息
                        new_windows_server = Server(public_ip=public_ip, private_ip=private_ip, hostname=hostname, port=3389, idc=self.account,
                                                    region=region,
                                                    state=self.state)
                        ...
                        new_server = Server(public_ip=public_ip, private_ip=private_ip, hostname=hostname, port=22, idc=self.account,
                                            region=region,
                                            state=self.state, admin_user=self.default_admin_user)
                        session.add(new_server)

                exist_ip = session.query(ServerDetail).filter(ServerDetail.publicip == public_ip).first()
                if exist_ip:
                    session.query(ServerDetail).filter(ServerDetail.public_ip == public_ip).update(
                        {ServerDetail.instance_id: instance_id, ServerDetail.instance_type: instance_type,
                         ServerDetail.instance_state: instance_state})
                else:
                    if os_type == 'windows':

                        new_serve_detail = ServerDetail(public_ip=public_ip, private_ip=private_ip, instance_id=instance_id, instance_type=instance_type,
